...
export default function SpecificCategory() {
    ...
    const handledetail = async (detailImdbIdSecond) => {
        try {
            const response = await fetch(`https://omdbapi.com/?i=${detailImdbIdSecond}&apikey=eae86d55`);
            const data = await response.json();
            if (data.Response === 'True') {
                navigate(`/detailmain/${detailImdbIdSecond}`);
                console.log(data)
            } else {
